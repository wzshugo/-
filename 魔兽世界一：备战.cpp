...
int main() {
	...
	for (int i = 1; i <= Case; i++) {
		...
		int volume, r_v, b_v, r_n = 1, b_n = 1; //总生命值，红方生命值，蓝方生命值,红方武士总数量，蓝方武士总数量
		...
		bool if_r_stop = false; //红方的制造是否停止
		bool if_b_stop = false; //蓝方的制造是否停止
		...
		int max_attempts = 5;
		int r_attempts = 0;  //红方尝试次数
		int b_attempts = 0;  //蓝方尝试次数
		...
		while ((!if_r_stop) || (!if_b_stop)) { //如果红方和蓝方的制造都还没停止
			...
			while (!if_b_stop) { //蓝方制造武士
				...
				if (!if_next(b_v, b_current->vol)) { //如果能制造当前的武士
